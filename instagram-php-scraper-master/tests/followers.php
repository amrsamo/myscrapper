...
$files = array('airlinefollowers','klm','tntairways');

foreach ($files as $file) {

	$file = file_get_contents('followers/airline/'.$file.'.log');
	getfollowers($file);
}


function getfollowers($file)
{
	...
	// $usernames = checkUsernames($usernames);
...
{	
	...
	$sql = "SELECT username FROM mails_scrap where username in ('".$usernames."')";
	...
	if ($result->num_rows > 0) {
	    ...
	    while($row = $result->fetch_assoc()) {
	    	$remove_index = array_search($row['username'], $output);
	    	unset($output[$remove_index]);
	    }
	} else {
	    return $output;
	}
...
function saveTayoMails($data, $user_data)
{
    ...
    foreach ($data as $mail) {
        $sql = "INSERT INTO user (email,username,url,followers,hashtag, externalUrl, location,instagram_unique_id,fullName,profilePicUrl ,biography,followsCount,mediaCount,isPrivate,isVerified)
                VALUES ('".$mail."','".$user_data['username'].
                           "','".$user_data['url'].
                           "',".$user_data['followers'].",
                           '".$user_data['hashtag']."',
